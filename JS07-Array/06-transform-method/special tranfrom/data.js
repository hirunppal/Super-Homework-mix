...
console.log(sales.length + " transactions");
// 2. จำนวนลูกค้าที่แตกต่างกัน มีใครบ้าง แต่ละคนซื้อไปยอดรวมกันเท่าไหร่ กี่เครื่อง
ClislistNumREV = () => {};
// 3. ยอดขายทั้งหมด (หลังหัก discount)
let NetRev = sales.map(function (element, name) {
  if (element.discount > 0) {
    let net = element.product.unitPrice * (1 - element.discount);
    return net;
  } else return element.product.unitPrice;
});
console.log(NetRev);

let sumnetrev = NetRev.reduce(function (acc, element) {
  return acc + element;
  console.log(element);
}, 0);
console.log(sumnetrev);
